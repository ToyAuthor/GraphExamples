...
sModelVertexChunk_OpenGL::~sModelVertexChunk_OpenGL()
{
	...
	if ( m_pVertexBuffer )
	{
		delete [] static_cast<unsigned char*>(m_pVertexBuffer);  // 我這樣寫可能不大安全
...
void sModelMaterial_OpenGL::Submit(sModelVertexChunk_OpenGL *pVChunk, unsigned int mask)
{
	...
	if ( pVChunk )
	{
		if ( !(glBindBuffer && pVChunk->m_VertexBufferID) )
		{
			...
			vmem = (uintptr_t) pVChunk->m_pVertexBuffer;
		}
	}
		...
		for ( i=0; i<m_iNumMaterials; i++, target++, source++ )
		{
			...
			for ( j=0; j<MAX_NUM_TEXTURES; j++ )
			{
				if ( source->m_szTexture[j][0] )
				{
					...
					{
						printf("We still need GutLoadCubemapTexture_OpenGL()\n");
...
void CGutModel_OpenGL::Render(unsigned int mask)
{
	...
	for ( int i=0; i<m_iNumMeshes; i++ )
	{
		...
		for ( int j=0; j<pMesh->m_iNumVertexChunks; j++ )
		{
			...
			{
				// 沒有支援 Vertex/Index buffer object, 資料在主記體中.
				vmem = (uintptr_t) pVertexChunk->m_pVertexBuffer;
				imem = (uintptr_t) pVertexChunk->m_pIndexBuffer;
			}
